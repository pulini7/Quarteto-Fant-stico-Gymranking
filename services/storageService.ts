...
export const getLeaderboardData = async (): Promise<User[]> => {
    ...
    const users = data.map((u: any) => ({
        ...mapUserFromDB(u),
        checkIns: u.check_ins.map((c: any) => ({ id: c.id, date: c.date, photo: '', timestamp: '', likes: [] }))
    }));
...
export const performCheckIn = async (userId: string, photoBase64: string, caption: string = ''): Promise<User | null> => {
  ...
  if (fetchError) {
      // ... Local DB fallback logic (kept same as previous for brevity, assumption: local mode is fast enough)
