            ...
            <div className="flex flex-col items-end gap-2 animate-fade-in z-30">
                <div className="bg-slate-800 p-3 rounded-xl shadow-2xl border border-slate-700 flex flex-col gap-2 min-w-[200px]">
                    <div className="flex justify-between items-center mb-1">
                        ...
                        <button onClick={() => { setShowLoginArea(false); setError(null); }} className="text-slate-500 hover:text-white">✕</button>
                    </div>
                    <form onSubmit={handleGenericLoginSubmit} className="flex flex-col gap-2">
                        <input type="text" value={loginName} onChange={(e) => setLoginName(e.target.value)} placeholder="Nome do Usuário" className="bg-slate-900 text-white text-sm px-3 py-2 rounded border border-slate-600 focus:border-brand-primary outline-none" autoFocus />
                        <input type="password" value={loginPassword} onChange={(e) => setLoginPassword(e.target.value)} placeholder="Senha" className="bg-slate-900 text-white text-sm px-3 py-2 rounded border border-slate-600 focus:border-brand-primary outline-none" />
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-lg">
          <div className="text-center mb-10 animate-fade-in">
            <h1 className="text-4xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-brand-accent mb-2 leading-tight">QUARTETO FANTÁSTICO<br/>GYMRANKING</h1>
            <p className="text-slate-400 mt-4 text-lg">Quem está treinando hoje?</p>
            {error && !selectedUser && !showLoginArea && (
                 <p className="text-brand-danger text-xs mt-2 bg-red-900/20 inline-block px-3 py-1 rounded-full border border-red-900/50 animate-pulse">⚠️ {error}</p>
            )}
          </div>
               ...
               return (
                ...
                >
                  <div className="absolute top-3 right-3 z-10">
                      {isLocked ? (
                          <div className="bg-slate-800/80 text-brand-primary p-1.5 rounded-full backdrop-blur-md border border-slate-600 shadow-lg"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg></div>
                      ) : (
                          <div className="bg-brand-accent/20 text-brand-accent p-1.5 rounded-full backdrop-blur-md border border-brand-accent/40 shadow-lg animate-pulse"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 5v14"/><path d="M5 12h14"/></svg></div>
                      )}
                  </div>
          ...
          <div className="mt-12 text-center pt-6 opacity-50"><p className="text-xs text-slate-500 uppercase tracking-widest">GymRank v1.1 - Fast Mode</p></div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md animate-fade-in">
          <div className="bg-brand-card w-full max-w-sm rounded-3xl border border-slate-700 p-8 shadow-2xl relative">
            ...
            <div className="text-center mb-6">
              ...
              <h2 className="text-2xl font-bold text-white">{(!selectedUser.password) ? 'Configuração Inicial' : `Olá, ${selectedUser.name}!`}</h2>
              <p className="text-slate-400 text-sm mt-1">{(!selectedUser.password) ? 'Para sua segurança, defina uma senha exclusiva para seu perfil.' : 'Digite sua senha para confirmar que é você.'}</p>
            </div>
            <form onSubmit={handlePasswordSubmit} className="space-y-4">
              <div>
                <input type="password" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} placeholder={(!selectedUser.password) ? "Crie sua senha" : "Sua senha"} className="w-full bg-slate-900 border border-slate-700 rounded-xl p-4 text-center text-white text-lg focus:ring-2 focus:ring-brand-primary outline-none tracking-widest placeholder:text-slate-600 placeholder:tracking-normal" autoFocus />
              ...
              <div className="flex items-center justify-center gap-2 mb-2">
                 <input type="checkbox" id="modal-remember" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} className="w-4 h-4 accent-brand-primary cursor-pointer rounded bg-slate-800 border-slate-600 focus:ring-brand-primary" />
                 <label htmlFor="modal-remember" className="text-sm text-slate-400 cursor-pointer select-none">Manter conectado</label>
              </div>
              <Button type="submit" fullWidth variant={(!selectedUser.password) ? 'accent' : 'primary'} className="py-4 text-lg">{(!selectedUser.password) ? 'Registrar Senha' : 'Acessar GymRanking'}</Button>
            </form>
          </div>
        </div>
      )}
